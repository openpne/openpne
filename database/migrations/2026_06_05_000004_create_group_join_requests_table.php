...
/*
 * Pending join requests for Approval-policy groups (the OpenPNE 3 `community_member`
 * is_pre=1 rows). Kept separate from group_members so confirmed-member reads cannot
 * accidentally include applicants — the same split as friend_requests vs friendships.
 *
 * Approval moves a row here into group_members (delete + insert in one transaction),
