...
@section('content')
    {{-- Enter a new password. The email is read-only (carried from the reset link); the token
         is hidden. passowrdResetForm is OpenPNE 3's own typo for this box
         (passwordRecoveryCompleteSuccess.php); skins target it, so it is restored verbatim. --}}
    <x-classic.parts id="passowrdResetForm" name="form" :title="__('Password Recovery')">
        <form method="POST" action="{{ route('password.update') }}">
            @csrf
            <input type="hidden" name="token" value="{{ $token }}">
            <div class="block">{{ __('Please input your new password.') }}</div>
            <table>
                <tr>
