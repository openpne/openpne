...
@section('content')
    {{-- Pending requests: no OpenPNE 3 page behind this — its requests were answered from the
         notification center and the home cautions — so its ids do not carry over. The roster shape
         does: _partsManageList.php, a 76×76 photo over the name then one operation per cell. Those
         cells go unclassed, as connection/listSuccess.php's three do; the `delete` class sizes a
         lone operation cell, not a pair. --}}
    <x-classic.parts id="friend_requests_received" name="manageList" :title="__('Requests received')">
        @if ($received->isEmpty())
    ...
    <x-classic.parts id="friend_requests_sent" name="manageList" :title="__('Requests sent')">
        @if ($sent->isEmpty())
