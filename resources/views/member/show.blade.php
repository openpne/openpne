            ...
            @else
                <table class="listBox">
                    @if ($age !== null)
                        <tr>
                            <th>{{ __('Age') }}</th>
                            <td>{{ __(':age years old', ['age' => $age]) }}@if ($isSelf && ($caption = $owner->preference(\App\Support\PreferenceKey::AgeVisibility)->ownerCaption(\App\Features\Profile\AgeVisibility::allowsWebPublic())) !== null) ({{ __($caption) }})@endif</td>
                        </tr>
                    @endif
