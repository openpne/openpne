...
@section('content')
    <x-classic.parts id="searchCommunity" name="form" :title="__('Search %communities%')">
        ...
        <div class="moreInfo">
            <ul class="moreInfo">
                <li><a href="{{ route('group.edit') }}">{{ __('Create a new %community%') }}</a></li>
            </ul>
        </div>
    </x-classic.parts>
    ...
    @else
        @php
            $results = $groups->map(fn ($group) => [
                ...
                'name' => $group->name,
                'rows' => [
                    ['caption' => __('%Community% Name'), 'value' => $group->name],
                    ['caption' => __('Count of Members'), 'value' => (string) $group->members_count],
                    ['caption' => __('Description'), 'value' => (string) $group->description],
                ],
            ])->all();
        @endphp
        <x-classic.parts id="searchCommunityResult" name="searchResultList" :title="__('Search Results')">
            <x-classic.search-result-list :items="$results" :paginator="$groups->withQueryString()" />
        </x-classic.parts>
    @endif
@endsection
