 ...
 * a place read differently at a glance. Missing images fall back to an id-colored initial badge
 * (stable across renames). Items are pre-shuffled server-side; this renders in order.
 */
export function NineTable({ items, shape }: { items: RightRailItem[]; shape: 'round' | 'square' }) {
    ...
    return (
        <ul className="grid grid-cols-3 gap-2">
            {items.map((item) => {
                const bgColor = pickPaletteColor(item.id);
                return (
                    <li key={item.id}>
                        <Link href={item.href} className="group block text-center" title={item.name}>
                            {item.imageUrl ? (
                                ...
                                <span
                                    aria-hidden
                                    className={`inline-flex aspect-square w-full items-center justify-center text-base font-bold leading-none ${pickReadableTextColor(bgColor)} ${rounded}`}
                                    style={{ backgroundColor: bgColor }}
                                >
                                    {computeInitial(item.name)}
                                </span>
                            )}
                            <p className="mt-1 truncate text-[10px] text-muted-foreground transition group-hover:text-foreground">
                                {item.name}
                            </p>
                        </Link>
                    </li>
