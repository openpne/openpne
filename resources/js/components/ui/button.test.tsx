...
// Each button's class set before the axes existed, from the base with its radius, a variant, a size
// and the call site's className, compared as a set since cn() may reorder.
const BASE =
...
describe('each axis renders the class set its call sites used to add by hand', () => {
    test.each([
        ['a load-more at the top of a list', cn(BASE, OLD.ghost, OLD.sm, 'w-full rounded-none border-b border-border py-3 text-link hover:bg-muted hover:text-link sm:px-5'), cn(buttonVariants({ variant: 'row', size: 'row', edge: 'bottom' }))],
        ['a load-more at the foot of a list', cn(BASE, OLD.ghost, OLD.sm, 'w-full rounded-none border-t border-border py-3 text-link hover:bg-muted hover:text-link sm:px-5'), cn(buttonVariants({ variant: 'row', size: 'row', edge: 'top' }))],
