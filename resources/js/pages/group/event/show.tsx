...
export default function GroupEventShow() {
    ...
    const submit = (commentOnly: boolean) => {
        ...
        form.post(`/events/${event.id}/comments`, {
            forceFormData: true,
            preserveScroll: true,
    ...
    const deleteEvent = async () => {
        if (await confirm({ title: t('Delete this event?'), description: event.name, confirmLabel: t('Delete'), danger: true })) {
            router.post(`/events/${event.id}/delete`);
        }
    };
            ...
            <Panel bodyClassName="space-y-3">
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span className="ml-auto shrink-0">
                        <RowMenu items={canEdit ? [{ label: t('Edit'), icon: Pencil, href: `/events/${event.id}/edit` }, { label: t('Delete'), icon: Trash2, destructive: true, onSelect: () => void deleteEvent() }] : []} />
                    </span>
                </div>
                ...
                <dl className="grid grid-cols-[max-content_1fr] gap-x-4 gap-y-1 text-sm">
                    ...
                    <dd>
                        ...
                        {' '}
                        <Link href={`/events/${event.id}/members`} className="text-link hover:underline">
                            {t('See Member List')}
                        </Link>
                    </dd>
                </dl>
