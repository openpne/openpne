 ...
 *
 * The overrides are cached in three tiers so the hot path stays small: the identity / auth / gadget
 * settings (read on every request — sns_name() is shared to Modern too) never deserialize the large
 * design blobs. The Classic HTML-insertion / footer keys load only when a Classic page renders, and
 * the custom CSS document is isolated entirely (its own entry, served by the CSS endpoint), so it is
 * never pulled into either map.
 */
class SnsSettingService
{
    ...
    /** The custom CSS document, isolated so it never enters a shared map. */
    ...
    /** Whether a non-empty custom CSS document is set — the cheap check the layout uses to emit the <link>. */
    ...
    /** The stored custom CSS, or '' when unset (so the absent state is cached, not re-queried each render). */
