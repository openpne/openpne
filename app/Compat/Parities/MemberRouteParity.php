    ...
    public function screens(): array
    {
        return [
            ...
            'home' => [
                ...
                new ScreenElement('%community% join-request caution', L::Two, S::Ported, 'homeSuccess customize cautionAboutCommunityMemberPre → community/_cautionAboutCommunityMemberPre.php', 'one line per administered group with its own count, its link naming the group and worded like the %friend% line (the confirmation center is not ported), to that group\'s pending page (PendingJoinRequestCounts, shared with the Modern dashboard); OpenPNE 3 summed them into one line'),
            ...
            'profile' => [
                ...
                new ScreenElement('per-value visibility suffix on the owner\'s own view', L::Three, S::Ported, "_profileListBox.php appends ' (%my_friend%)' / ' (All Users on the Web)' when the viewer is the owner", "the captions are the editor's audience labels (%Friends% only / Anyone on the web) rather than OpenPNE 3's publicFlags catalogue; the Age suffix (Friends only, as OpenPNE 3 captioned the age) is the owner's alone where OpenPNE 3 showed it to a friend too"),
