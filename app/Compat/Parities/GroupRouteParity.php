...
class GroupRouteParity extends RouteParity
{
    ...
    {
        return [
            new RouteMap('community_home', '/community/:id', 'group.show', 'GET', op3Action: 'home'),
            new RouteMap('community_search', '/community/search', 'group.search', 'GET', op3Action: 'search'),
            // joinList is "groups this member belongs to"; ?id= shows another member's list.
            ...
            new RouteMap('community_quit', '/community/quit', 'group.quit', 'POST'),
            ...
            new RouteMap(null, null, 'group.members.demote.show', 'GET', op3Action: 'removeSubAdmin'),
            new RouteMap(null, null, 'group.members.demote', 'POST'),
            new RouteMap(null, null, 'group.members.drop.show', 'GET', op3Action: 'dropMember'),
            new RouteMap(null, null, 'group.members.drop', 'POST'),
            new RouteMap(null, null, 'group.members.transfer.show', 'GET', op3Action: 'changeAdminRequest', note: 'The nominee accepts or declines from a banner on the community home plus a feed notification — OpenPNE 4 has no confirmation center (OpenPNE 3 routed the decision through it).'),
            new RouteMap(null, null, 'group.members.transfer', 'POST'),
    ...
    public function compatRedirects(): array
    {
        ...
        return [
            ...
            '/community/joinList' => 'group.list_mine',
            '/community/edit' => 'group.edit',
            '/community/join' => 'group.join.show',
            '/community/quit' => 'group.quit.show',
            '/community/delete/:id' => 'group.delete.show',
            '/community/member/list' => 'group.members',
            '/community/member/manage/:id' => 'group.members.manage',
            '/community/member/pending' => 'group.members.pending',
            '/community/member/appointSubAdmin' => 'group.members.appoint.show',
            '/community/member/demoteSubAdmin' => 'group.members.demote.show',
            '/community/member/drop' => 'group.members.drop.show',
            '/community/member/transferAdmin' => 'group.members.transfer.show',
            '/community/recent' => 'group.recent',
        ];
    }
    ...
    public function screens(): array
    {
        return [
            ...
            'home' => [
                ...
                new ScreenElement('administrator + sub-administrator rows', L::Two, S::Ported, '$groupAdmin / $groupSubAdmins link_to member_profile'),
                ...
                new ScreenElement('recent topics list', L::Two, S::Ported, 'communityTopic/_communityTopicList.php tr.communityTopic ul.articleList', 'a communityHome table row again: last-comment date (bumped_at, where OpenPNE 3 drew updated_at), 36-width truncated title with comment count, More over a non-empty list, create link per authority'),
                new ScreenElement('recent events list', L::Two, S::Ported, 'communityEvent/_communityEventList.php tr.communityEvent ul.articleList', 'same shape as recent topics; the date is the last-comment date, not the open date'),
                ...
                new ScreenElement('community timeline widget', L::Two, S::Partial, 'timeline/_timelineCommunity.php (opTimelinePlugin homeSuccess customize)', 'the slot holds a #groupTalk link box (box kind, "Go to talk") into the Modern talk surface; the OpenPNE 3 #communityTimeline box, its compose form and rows are not rendered'),
