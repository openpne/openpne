    ...
    public function screens(): array
    {
        return [
            ...
            'sns' => [
                ...
                new ScreenElement('opTimelinePlugin stylesheets', L::Two, S::Partial, '_timelineAll use_stylesheet bootstrap.css + timeline.css + counter.css (component-driven)', 'bootstrap and timeline.css come from every timeline-rendering screen and gadget, counter.css only with the compose box (layout pluginCss stack, once per page); lightbox.css and jquery.colorbox.css stay unloaded with their scripts'),
            ...
            // redirects to group talk, which is Modern-only, so Classic has only a link box to offer.
            'community' => [
                new ScreenElement('%community% timeline box', L::One, S::Partial, 'opTimelinePlugin community/config/view.yml homeSuccess customize timelineCommunity (parts communityHome, target before) + _timelineCommunity.php div#communityTimeline.dparts.communityTimeline', 'the home slot survives as a link box (#groupTalk in group/show.blade.php) into the Modern talk screen; the timeline box itself renders on no Classic screen'),
                new ScreenElement('member-only rendering', L::One, S::Partial, '_timelineCommunity.php if ($community->isPrivilegeBelong($memberId)) + api/activity search forward400Unless CommunityMember::isMember', 'the link box follows GroupTalkAccess::canView, the group read column, so an Everyone group offers talk to a non-member; OpenPNE 3 drew the box for members only'),
