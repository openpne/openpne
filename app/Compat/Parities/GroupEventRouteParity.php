    ...
    public function screens(): array
    {
        return [
            ...
            'new' => [
                ...
                new ScreenElement('open date', L::Two, S::Partial, 'open_date opWidgetFormDate (month_format number, can_be_empty)', 'a native date input is kept on purpose; OpenPNE 3 drew a year input with month / day selects'),
                new ScreenElement('open-date supplement input', L::Two, S::Ported, 'open_date_comment sfWidgetFormInput', 'its own labelled row after the open date'),
                new ScreenElement('area input', L::Two, S::Ported, 'area sfWidgetFormInput'),
                new ScreenElement('application deadline', L::Two, S::Partial, 'application_deadline opWidgetFormDate', 'a native date input is kept on purpose; OpenPNE 3 drew a year input with month / day selects'),
                new ScreenElement('capacity input', L::Two, S::Ported, 'capacity sfValidatorInteger (min 0)'),
                new ScreenElement('image upload (×3)', L::Two, S::Ported, 'photo_1..3 embedded opCommunityTopicPluginImageForm, ul#community_event_photo_N', 'x-classic.photo-rows: one labelled row per slot, the input inside ul#community_event_photo_N'),
                new ScreenElement('field order', L::Three, S::Ported, 'PluginCommunityEventForm setup() order: title, body, open date, supplement, area, deadline, capacity', 'title, body, open date, supplement, area, deadline, capacity, photos'),
            ...
            'communityEventComment/deleteConfirm' => [
                ...
                new ScreenElement('form table', L::Three, S::Deferred, '_partsForm.php <table> (the confirm binds a bare sfForm, so the table holds no field row)', 'an empty table with nothing to style; single-action confirmations stay free-form (docs/internals/classic-compatibility.md, "Default skin"), as the event delete confirm does'),
