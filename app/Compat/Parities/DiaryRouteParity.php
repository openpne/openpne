    ...
    public function screens(): array
    {
        return [
            ...
            'new' => [
                ...
                new ScreenElement('remembered default visibility', L::Three, S::Ported, 'MemberConfigDiaryForm::PUBLIC_FLAG default', 'DiaryVisibility::defaultFor — the diary_default_visibility preference the %diary% settings category writes, pre-selected on the form'),
            ...
            'search' => [
                ...
                new ScreenElement('no-match message naming the keyword', L::Three, S::Ported, "op_include_box('diaryList', 'Your search \"%1%\" did not match any diaries.')"),
                new ScreenElement('search window limited to the last N days', L::Two, S::Ported, 'getDiarySearchPager op_diary_plugin_search_period_enable / op_diary_plugin_search_period', 'SnsSettingKey::DiarySearchPeriodEnabled / DiarySearchPeriodDays through DiarySearch::periodLowerBound(), the same midnight-N-days-ago lower bound on this screen alone'),
                new ScreenElement('site-wide search switch', L::Two, S::Ported, 'executeSearch forward404Unless(SnsConfig op_diary_plugin_search_enable) + $isSearchEnable around the form line', 'SnsSettingKey::DiarySearchEnabled: the screen 404s and the form line leaves the list on both surfaces'),
            ],
