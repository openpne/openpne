...
final class PublishHomeIssue
{
    ...
    /**
     * Publish today's issue, or null on a 休刊.
     *
    ...
    {
        $existing = $this->publishedOn($now);
        if ($existing !== null) {
            return $existing;
        }

        $plan = $this->plan($now, $pin);
        if ($plan === null) {
            return null;
        }
    ...
    /**
     * Today's issue if it has already been published.
     *
     ...
     */
    public function publishedOn(CarbonImmutable $now): ?HomeIssue
    {
        return HomeIssue::query()->whereDate('issue_date', $now->toDateString())->first();
    }
     ...
     */
    public function plan(CarbonImmutable $now, ?SourceRef $pin = null): ?HomeIssuePlan
    {
