...
#[Description('The group talk rooms you belong to, the one talked in most recently first, each with how many messages you have not read and how many of those address you.')]
#[IsReadOnly]
class ListTalkRoomsTool extends TalkTool
{
    public function handle(Request $request, JoinedTalkRooms $rooms): ResponseFactory
    {
        ...
        // resolver would answer page one every time. The mention count is asked for here and nowhere
        // else — it is the whole reason an agent can poll one call instead of reading every room.
        $page = $rooms(
            $this->member($request),
