...
/**
 * Counts the source term rows the TermOverrideUpgrade INSERT would fail on mid-run: two rows that
 * fold onto one `term_overrides` (name, locale) key (OpenPNE 3 makes neither `sns_term.name` nor the
 * folded lang unique), and a value wider than the target column
 * (docs/internals/upgrade.md, "Source preflight").
 */
