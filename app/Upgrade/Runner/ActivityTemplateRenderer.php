...
final class ActivityTemplateRenderer
{
    ...
    {
        $params = self::params($serializedParams);
        if ($params === null) {
            return ['body' => null, 'reason' => self::BAD_PARAMS];
        }

        $url = ActivityUriMapper::resolve($uri);
        if ($url === null) {
        ...
        if (! in_array($template, ActivityPreflight::KNOWN_TEMPLATES, true)) {
    ...
    /**
     * `%1%` => 'x' as a 1-based list of strings, or null when the value is not that. Objects are
     * refused and a malformed string is a notice, which the handler turns into null.
     *
     * @return array<int, string>|null
     */
    private static function params(?string $serialized): ?array
    {
        ...
        foreach ($decoded as $key => $value) {
            if (! is_scalar($value) || ! preg_match('/^%([1-3])%$/', (string) $key, $m)) {
                return null;
            }
