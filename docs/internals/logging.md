...
| `passkey.failed` | seam: `App\Features\Auth\PasskeyLoginController::store` — an assertion the ceremony refused (unknown credential, stale challenge, bad signature); the gate's refusal is not doubled here | `guard` |
