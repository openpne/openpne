...
| `passkey.failed` | seam: `App\Features\Auth\PasskeyLoginController::store` — an assertion the ceremony refused (unknown credential, stale challenge, bad signature, a counter that went backwards); the gate's refusal is not doubled here | `guard`, `credential_id` |
...
| `ai_account.created` / `ai_account.deleted` | seam: `CreateAiAccount` / `DeleteAiAccount` (actions) | `member_id`, `owner_id` |
