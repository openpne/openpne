...
free text) renders through a single dispatch per surface — never ad-hoc escaping at call
sites. Three formats exist, carried per record in a `format` column on `diaries`,
...
[`app/Upgrade/Runner`](../../app/Upgrade/Runner)), normalized to RGI fully-qualified
sequences precisely because the rewrite is irreversible. Third-party data attribution for the
mapping lives in the repository [NOTICE](../../NOTICE).
...
- **The sanitizer's default input cap is disabled.** It silently truncates past 20 KB; a safe
  ...
  every body-authoring rule and the preview cap the **raw** input at the TEXT column's 65,535
  bytes ([`MaxBytes`](../../app/Rules/MaxBytes.php) — bytes, not characters).
...
- One dispatch: body HTML comes from `BodyRenderer`; excerpt/mail text come from its
  `excerpt`/`plainText`. No call site escapes or renders a body by hand.
