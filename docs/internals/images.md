...
takes a 48 MP photo whole. A favicon the processor refuses is remembered as such
...
| Source limits | a header over `max_upload_dimension` a side or `max_source_pixels` (25 MP unconfigured) is refused before the decode | no per-side limit; the pixel limit is the sidecar's 50 MP budget unless `max_source_pixels` is set |
...
read; with them absent, iOS transcodes a HEIC to JPEG itself, as it always has.
