...
3. A migration that adds a column the running code writes runs with the site in maintenance mode
   (`php artisan down`), because the code before and after it cannot share the table
   ([group-boards.md](group-boards.md#the-board-key-is-bumped_at) is one).
