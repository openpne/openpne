...
opted in the audience is narrowed to the opt-ins. That bound depends on `Member::setNotificationSetting`
deleting, for a kind with a site default, a row equal to that default rather than storing it, so the
rows are genuine opt-ins and not every member who ever saved the settings form. It is dispatched with a short delay so a member
