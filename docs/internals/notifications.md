...
*live* factor (`DisableMemberMfa` reports `wasEnabled`), so cancelling a pending set-up — and the
operator lockout CLI acting on a member with no active factor — sends nothing.
