...
model save, even a quiet one, bumps `updated_at`. The column's `DEFAULT CURRENT_TIMESTAMP` exists only
so MySQL adds no `ON UPDATE` clause; every write path sets the value, and the default is never what a
row means.
