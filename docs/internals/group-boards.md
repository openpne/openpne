...
A list row shows `bumped_at` under the label its count decides, "Last comment" with comments and
"Posted" without, since after the last comment goes the two are the same instant; the detail shows
`created_at` with an "Edited" mark when `edited_at` is set. The two screens name different instants
on purpose, and the labels are what keep them from reading as one.
