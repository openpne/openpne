...
2. Under those locks the image Files of the talk, the topics, the events and their comments are
   ...
   the group id and the reactions are found a page of a thousand comments at a time and deleted by
   primary key in chunks, so neither a statement nor what is held in PHP grows with the group, and the
   sweep locks only the rows it deletes. The group's own top-image
