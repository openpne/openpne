...
- **Member-authored emphasis** — `<strong>` in a body, and the `.rich-body` rules in `app.css`. The
  writer's emphasis, not the interface's.
...
- The guard reads Tailwind classes in `.tsx`, including arbitrary `font-[…]`. Plain CSS, inline
