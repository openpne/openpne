...
noun; `I18nTermLiteralTest` pins `lang/ja.json` to that). The mobile application's rows (half-width kana) and
unrecognised names are not migrated, the latter reported by the unknown-name scan; an empty or
