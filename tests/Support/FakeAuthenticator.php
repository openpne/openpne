...
{
    /** Not in the bundled table, so a registration carries no name unless a test sets one. */
    public string $aaguid = "\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f\x10";
    ...
    private function authData(string $rpId, bool $attested): string
    {
        ...
        if ($attested) {
            ...
            $authData .= $this->aaguid.pack('n', strlen($id)).$id.$this->coseKey();
        }
