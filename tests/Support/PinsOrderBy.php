...
trait PinsOrderBy
{
    ...
    /**
     * @param  callable(): mixed  $run
     * @return list<string> every query on $table, quotes stripped
     */
    protected function querySqlOn(string $table, callable $run): array
    {
        DB::flushQueryLog();
        DB::enableQueryLog();
        try {
        ...
        return $sql
            ->filter(fn (string $q) => preg_match('/from [`"]'.preg_quote($table, '/').'[`"]/', $q) === 1)
            ->map(fn (string $q) => preg_replace('/[`"]/', '', $q))
            ->values()->all();
    }
}
