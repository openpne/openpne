...
class DirectMessageRecipientUpgradeSqlTest extends TestCase
{
    ...
    public function test_a_read_receipt_sets_read_at(): void
    {
        ...
        $this->assertDatabaseHas('direct_message_recipients', ['id' => 901, 'read_at' => '2019-09-09 09:09:09']);
    }
    ...
    public function test_a_draft_send_list_row_creates_no_receipt(): void
    {
        ...
        $this->assertDatabaseMissing('direct_message_recipients', ['id' => 910]);
    }

    public function test_an_orphan_send_list_row_is_quarantined(): void
    {
        $recipient = Member::factory()->create();
        // No parent message id=99999, so the direct_message_id FK could not hold; the filter drops it.
        $this->seedSendList(920, $recipient->id, 99999);

        $this->runUpgrade();

        $this->assertDatabaseMissing('direct_message_recipients', ['id' => 920]);
    }
    ...
    public function test_a_non_personal_parent_send_list_is_skipped(): void
    {
        ...
        $this->assertDatabaseMissing('direct_message_recipients', ['id' => 930]);
    }
    ...
    public function test_a_withdrawn_recipient_keeps_the_receipt_with_null_member(): void
    {
        ...
        $this->assertNull(DirectMessageRecipient::findOrFail(950)->recipient_id);
    }

    private function runUpgrade(): void
    {
        $this->runStep(new DirectMessageUpgrade);
        $this->runStep(new DirectMessageRecipientUpgrade);
    }
