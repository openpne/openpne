...
class NavigationUpgradeSqlTest extends TestCase
{
    ...
    public function test_maps_the_community_type_and_tokens_into_the_group_space(): void
    {
        ...
        $this->seedNav(5, 'community', 'communityEvent/listCommunity');
        $this->seedNav(6, 'community', '@communityEvent_new');

        $this->runUpgrade();

        $rows = DB::table('navigations')->whereIn('id', [1, 2, 3, 4, 5, 6])->get()->keyBy('id');
        ...
        $this->assertSame('communityEvent/listCommunity', $rows[5]->source_uri);
        ...
        $this->assertSame('/groups/:id/events', $rows[5]->uri);
        $this->assertSame('/groups/:id/events/new', $rows[6]->uri);
    }
