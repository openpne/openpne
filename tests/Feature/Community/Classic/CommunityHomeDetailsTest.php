...
class CommunityHomeDetailsTest extends TestCase
{
    ...
    {
        $community = Community::factory()->create(['name' => 'Tokyo Walkers']);
        $member = Member::factory()->create();
        CommunityMember::factory()->member()->create(['community_id' => $community->id, 'member_id' => $member->id]);
    ...
    public function test_the_recent_event_and_topic_rows_close_the_details_table(): void
    {
        ...
        $member = Member::factory()->create();
        CommunityMember::factory()->member()->create(['community_id' => $community->id, 'member_id' => $member->id]);
        $topic = CommunityTopic::factory()->create(['community_id' => $community->id, 'name' => 'Walk plans', 'member_id' => $member->id]);
        $topic->timestamps = false;
        $topic->forceFill(['updated_at' => '2026-07-08 12:00:00'])->save();
        $event = CommunityEvent::factory()->create(['community_id' => $community->id, 'name' => 'Summer stroll', 'member_id' => $member->id]);
        ...
        $response->assertSeeInOrder([
            '<tr class="communityTopic">',
            '<div class="moreInfo">',
            route('communityTopic.index', $community),
            route('communityTopic.new', $community),
        ], false);
    }
    ...
    public function test_an_empty_board_row_keeps_the_create_link_but_drops_more(): void
    {
        ...
        $response->assertSee(route('communityTopic.new', $community), false);
    }
    ...
    public function test_a_viewer_without_post_authority_gets_no_create_link(): void
    {
        ...
        $member = Member::factory()->create();
        CommunityMember::factory()->member()->create(['community_id' => $community->id, 'member_id' => $member->id]);
        CommunityTopic::factory()->create(['community_id' => $community->id, 'name' => 'Members read this', 'member_id' => $member->id]);

        $response = $this->actingAs($member)->get(route('community.show', $community))->assertOk();

        $response->assertSee('<tr class="communityTopic">', false);
        $response->assertSee(route('communityTopic.index', $community), false);
        $response->assertDontSee(route('communityTopic.new', $community), false);
    }
