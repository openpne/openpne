 ...
 */
class BroadcastCommunityNewPostTest extends TestCase
{
    use RefreshDatabase;

    private function member(Community $community, ?Member $member = null, CommunityRole $role = CommunityRole::Member): Member
    {
    ...
    public function test_a_new_topic_notifies_confirmed_members_but_not_the_author(): void
    {
        ...
        $author = $this->member($community, role: CommunityRole::Admin);
    ...
    public function test_a_member_without_an_address_gets_the_feed_row_but_no_mail(): void
    {
        ...
        $community = Community::factory()->create();
        $author = $this->member($community);
        $noAddress = $this->member($community, Member::factory()->create(['email' => null]));
        $topic = CommunityTopic::factory()->create(['community_id' => $community->getKey(), 'member_id' => $author->getKey()]);
