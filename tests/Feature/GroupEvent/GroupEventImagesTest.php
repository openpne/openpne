...
class GroupEventImagesTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function form(): GroupEventFormData
    {
    ...
    public function test_an_event_is_created_with_numbered_images_it_owns_and_shows_them(): void
    {
        ...
        $this->actingAs($member)->post(route('group.events.store', $group), $this->eventPayload([
            'name' => 'With pics',
            'images' => [$this->fake('a.png'), $this->fake('b.png')],
        ]))->assertRedirect();

        $event = GroupEvent::where('name', 'With pics')->firstOrFail();
        ...
        $this->assertSame('groupEvent', $file->related_entity_type);
        $this->assertSame($event->getKey(), $file->related_entity_id);

        $this->actingAs($member)->get(route('group.events.show', $event))
            ->assertOk()
    ...
    public function test_a_comment_is_posted_with_images_owned_by_the_comment(): void
    {
        ...
        $this->assertSame('groupEventComment', $file->related_entity_type);
    ...
    public function test_deleting_an_event_purges_its_and_its_comments_image_bytes(): void
    {
        ...
        $this->assertDatabaseCount('group_event_images', 0);
        $this->assertDatabaseCount('group_event_comment_images', 0);
    }
    ...
    public function test_a_failed_image_in_a_merged_participate_rolls_back_the_join_too(): void
    {
        ...
        $this->assertDatabaseCount('group_event_comments', 0); // the comment rolled back
        $this->assertDatabaseCount('group_event_comment_images', 0);
        $this->assertDatabaseCount('files', 0);
