...
class GroupEventRoutesTest extends TestCase
{
    ...
    public function test_guests_are_redirected_to_login(): void
    {
        ...
        $this->get(route('group.events.new', $group))->assertRedirect('/login');
        $this->get(route('group.events.show', $event))->assertRedirect('/login');
        $this->get(route('group.events.member_list', $event))->assertRedirect('/login');
        $this->post(route('group.events.store', $group))->assertRedirect('/login');
        $this->post(route('group.events.delete', $event))->assertRedirect('/login');
    }
    ...
    public function test_modern_index_renders_the_board(): void
    {
        ...
        GroupEvent::factory()->create(['group_id' => $group->getKey(), 'member_id' => $member->getKey()]);

        $this->actingAs($member)
            ->get(route('group.events.index', $group))
            ->assertInertia(fn ($page) => $page
                ->component('group/event/index')
    ...
    public function test_modern_show_renders_the_event_with_rsvp_state(): void
    {
        ...
        GroupEventComment::factory()->create(['group_event_id' => $event->getKey(), 'member_id' => $author->getKey(), 'number' => 1]);

        $this->actingAs($author)
            ->get(route('group.events.show', $event))
            ->assertInertia(fn ($page) => $page
    ...
    public function test_modern_new_renders_the_form_for_a_member(): void
    {
        ...
        $this->actingAs($member)
            ->get(route('group.events.new', $group))
            ->assertInertia(fn ($page) => $page
                ->component('group/event/edit')
                ->where('group.id', $group->getKey())
                ->where('event', null)
    ...
    public function test_modern_new_returns_404_for_a_non_member(): void
    {
        ...
        $this->actingAs($stranger)->get(route('group.events.new', $group))->assertNotFound();
    }
    ...
    public function test_modern_store_creates_an_event_and_redirects_to_show(): void
    {
        ...
        $event = GroupEvent::where('name', 'Modern Meetup')->firstOrFail();
    ...
    public function test_modern_edit_renders_the_form_with_ymd_dates_for_the_author(): void
    {
        ...
        $this->actingAs($author)
            ->get(route('group.events.edit', $event))
            ->assertInertia(fn ($page) => $page
                ->component('group/event/edit')
                ->where('event.id', $event->getKey())
    ...
    public function test_modern_edit_returns_404_for_a_non_editor(): void
    {
        ...
        $this->actingAs($stranger)->get(route('group.events.edit', $event))->assertNotFound();
    }
    ...
    public function test_modern_update_edits_the_event_and_redirects_to_show(): void
    {
        ...
        $this->actingAs($author)
            ->post(route('group.events.update', $event), $this->eventPayload(['name' => 'Renamed', 'area' => 'Osaka']))
            ->assertRedirect(route('group.events.show', $event));

        $this->assertDatabaseHas('group_events', ['id' => $event->getKey(), 'name' => 'Renamed', 'area' => 'Osaka']);
    }
    ...
    public function test_modern_delete_removes_the_event_and_redirects_to_the_community(): void
    {
        ...
        $this->actingAs($author)
            ->post(route('group.events.delete', $event))
    ...
    public function test_modern_delete_returns_404_for_a_non_editor(): void
    {
        ...
        $this->actingAs($stranger)->post(route('group.events.delete', $event))->assertNotFound();
        $this->assertDatabaseHas('group_events', ['id' => $event->getKey()]);
    }
    ...
    public function test_modern_member_list_renders_the_roster(): void
    {
        ...
        $this->actingAs($member)
            ->get(route('group.events.member_list', $event))
            ->assertInertia(fn ($page) => $page
                ->component('group/event/members')
                ->where('group.id', $group->getKey())
                ->where('event.id', $event->getKey())
    ...
    public function test_modern_only_serves_the_canonical_event_board_as_inertia(): void
    {
        ...
        $this->actingAs($member)
            ->get(route('group.events.index', $group))
            ->assertInertia(fn ($page) => $page->component('group/event/index'));
    }
}
