...
class CommunityEventRoutesTest extends TestCase
{
    ...
    {
        $group = Group::factory()->create();
        $stale = CommunityEvent::factory()->create(['community_id' => $group->getKey(), 'name' => 'Stale event']);
        $fresh = CommunityEvent::factory()->create(['community_id' => $group->getKey(), 'name' => 'Fresh event']);
        DB::table('community_events')->where('id', $stale->getKey())->update(['updated_at' => now()->subDays(3)]);

        $response = $this->actingAs($this->joined($group))->get(route('communityEvent.index', $group));
    ...
    {
        $group = Group::factory()->create();
        $event = CommunityEvent::factory()->create(['community_id' => $group->getKey(), 'name' => 'Counted']);
    ...
    public function test_board_draws_the_openpne3_recent_list(): void
    {
        ...
        $event = CommunityEvent::factory()->create([
            'community_id' => $group->getKey(), 'name' => 'A meetup',
    ...
    public function test_members_only_board_is_hidden_from_non_members(): void
    {
        ...
        $this->actingAs($stranger)->get(route('communityEvent.index', $group))->assertNotFound();
        ...
        $this->actingAs($this->joined($group))->get(route('communityEvent.show', $event))->assertOk();
    }
    ...
    public function test_show_renders_event_with_body_id_and_scheduling_details(): void
    {
        ...
        $response = $this->actingAs($this->joined($group))->get(route('communityEvent.show', $event));
    ...
    public function test_show_autolinks_a_url_in_the_area(): void
    {
        ...
        $this->actingAs($this->joined($group))->get(route('communityEvent.show', $event))
            ->assertOk()
    ...
    public function test_new_event_is_admin_only_when_posting_is_restricted(): void
    {
        ...
        $this->actingAs($member)->get(route('communityEvent.new', $group))->assertNotFound();
        $this->actingAs($member)->post(route('communityEvent.store', $group), $this->eventPayload())->assertNotFound();

        $this->actingAs($admin)->get(route('communityEvent.new', $group))
            ->assertOk()
    ...
    public function test_an_unauthorized_poster_gets_404_even_with_an_invalid_payload(): void
    {
        ...
        $this->actingAs($stranger)->post(route('communityEvent.store', $group), ['name' => '', 'body' => ''])
            ->assertNotFound();
    ...
    public function test_member_list_shows_participants_with_body_id(): void
    {
        ...
        $response = $this->actingAs($this->joined($group))->get(route('communityEvent.member_list', $event));
    ...
    {
        $group = Group::factory()->create(['topic_read_access' => TopicReadAccess::MembersOnly]);
        CommunityEvent::factory()->create(['community_id' => $group->getKey(), 'name' => 'Box event']);

        $this->actingAs($this->joined($group))->get(route('group.show', $group))
            ->assertOk()
            ->assertSee('Box event')
            ->assertSee(route('communityEvent.index', $group), false);
