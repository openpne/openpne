...
class DirectMessageComposeTest extends TestCase
{
    ...
    public function test_sending_delivers_to_the_inbox_and_notifies(): void
    {
        ...
        $message = DirectMessage::firstOrFail();
    ...
    public function test_draft_edit_renders_for_the_owner_and_sends(): void
    {
        Notification::fake();
        [$sender, $recipient] = Member::factory()->count(2)->create();
        $draft = app(SendDirectMessage::class)($sender, new DirectMessageComposeData($recipient->getKey(), 'Draft', 'Body'), asDraft: true);
    ...
    {
        [$sender, $recipient] = Member::factory()->count(2)->create();
        $draft = app(SendDirectMessage::class)($sender, new DirectMessageComposeData($recipient->getKey(), 'Draft', 'Body'), asDraft: true, images: [
    ...
    public function test_a_reply_link_to_a_message_the_viewer_is_not_party_to_is_rejected(): void
    {
        ...
        $foreign = app(SendDirectMessage::class)($stranger, new DirectMessageComposeData($other->getKey(), 'X', 'Y'), asDraft: false);
        ...
        $this->assertSame(1, DirectMessage::count()); // nothing new sent
    }
    ...
    public function test_a_reply_link_to_a_received_message_is_accepted(): void
    {
        Notification::fake();
        [$sender, $recipient] = Member::factory()->count(2)->create();
        $original = app(SendDirectMessage::class)($sender, new DirectMessageComposeData($recipient->getKey(), 'Orig', 'Body'), asDraft: false);
