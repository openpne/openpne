 ...
 * without one.
 */
class GroupTopicCommentReplyLinkTest extends TestCase
{
    use RefreshDatabase;
    ...
    /** @return array{0: GroupTopic, 1: Member, 2: Member} the record, a member who may comment, the comment's author */
    private function threadWithOneComment(): array
    {
        $group = Group::factory()->create();
        $member = $this->joined($group);
        $author = $this->joined($group);
        $record = GroupTopic::factory()->create(['group_id' => $group->getKey(), 'member_id' => $member->getKey()]);
        GroupTopicComment::factory()->create(['group_topic_id' => $record->getKey(), 'member_id' => $author->getKey(), 'number' => 1]);
    ...
    public function test_no_reply_link_while_the_switch_is_off(): void
    {
        ...
        $response = $this->actingAs($member)->get(route('group.topics.show', $record));
    ...
    public function test_a_viewer_who_cannot_comment_gets_no_reply_link(): void
    {
        ...
        $response = $this->actingAs($stranger)->get(route('group.topics.show', $record));
